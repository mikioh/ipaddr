...
func TestSummarize(t *testing.T) {
	for i, tt := range summarizeTests {
		...
		sort.Sort(byAddrAndLen(wants))
		sums := ipaddr.Summarize(first, last)
		if !reflect.DeepEqual(sums, wants) {
			t.Errorf("#%v: got %v; want %v", i, sums, wants)
		}
	}
}

type byAddrAndLen []ipaddr.Prefix
...
func (ps byAddrAndLen) Less(i, j int) bool {
	if ncmp := ipaddr.Compare(ps[i], ps[j]); ncmp < 0 {
		return true
	} else if ncmp > 0 {
		return false
	}
