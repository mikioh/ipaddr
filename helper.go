...
func (i ipv4Int) encodeNLRI(b []byte, nbits int) int {
	l := nbits / 8
	if nbits%8 > 0 {
		l++
	}
...
func (i *ipv6Int) lshift(nbits int) {
	i[0] = i[0]<<uint(nbits) | i[1]>>uint(64-nbits) | i[1]<<uint(nbits-64)
	i[1] = i[1] << uint(nbits)
}

func (i *ipv6Int) rshift(nbits int) {
...
func (a *ipv6Int) compare(b *ipv6Int) int {
	if a[0] < b[0] {
		return -1
	} else if a[0] > b[0] {
		return +1
...
func (i *ipv6Int) setHostmask(nbits int) {
	if nbits > 64 {
...
func (i *ipv6Int) setNetmask(nbits int) {
	if nbits > 64 {
		i[0], i[1] = mask64(64), mask64(nbits-64)
	} else {
...
func (i *ipv6Int) encodeNLRI(b []byte, nbits int) int {
	l := nbits / 8
	if nbits%8 > 0 {
		l++
	}
